...
class Sub(var x: Int, var depth: Int) {
  override def toString = s"$x, $depth, ${x * depth}"
  def move(movement: Vector): Sub = movement match {
  	case Vector(Directions.up, magnitude) => new Sub(x, depth - magnitude)
  	case Vector(Directions.down, magnitude) => new Sub(x, depth + magnitude)
  	case Vector(Directions.forward, magnitude) => new Sub(x + magnitude, depth)
...
object Main {
    ...
    def main(args: Array[String]): Unit = {
    	cleanly(Source.fromFile(args(0))) { fin =>
    	  fin.getLines()
    	      .map(parsedVector)
    	      .foldLeft(new Sub(0, 0))((a: Sub, c: Vector) => a.move(c))
		} match {
